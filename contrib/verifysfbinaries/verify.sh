...
###   This script attempts to download the signature file SHA256SUMS.asc from nur1labs.org
...
WORKINGDIR="/tmp/mubdi"
TMPFILE="hashes.tmp"

#this URL is used if a version number is not specified as an argument to the script
SIGNATUREFILE="https://nur1labs.net/bin/0.9.2.1/SHA256SUMS.asc"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://nur1labs.org/bin/"
VERSIONPREFIX="mubdi-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'mubdi-',
...
if ! WGETOUT=$(wget -N "$BASEDIR$SIGNATUREFILENAME" 2>&1); then
   ...
   echo "[mubdi-]<version>-[rc[0-9]] (example: mubdi-0.9.2-rc1)"
   echo "wget output:"
