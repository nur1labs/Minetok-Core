...
The RPM spec file provided here is for MuBdI-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://nur1labs.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[MuBdI git repository](https://github.com/nur1labs/mubdi) but are not part of
...
Currently only `Source10` is used, for the example `mubdi.conf` file.
...
in the mubdi git repository. At present only an SVG version of the MuBdI
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the mubdi
...
needed when 0.12.1 is released, a proper fix is already in the MuBdI git
master branch.
...
idea to build MuBdI Core against a static version of that library compiled
...
The `mubdi.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `mubdi-server` package. At this point, most RPM based
...
The `mubdi-server` package should have SELinux support. How to properly do
...
MuBdI-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/mubdi/). He has been
packaging MuBdI for Fedora at least since 2012.
...
4. I renamed the `mubdi` package that contains the Qt GUI to `mubdi-core` as
that appears to be how the general population refers to it, in contrast to
`mubdi-xt` or `mubdi-classic`. I wanted to make sure the general population
