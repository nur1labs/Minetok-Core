...
*Setup instructions for a gitian build of MuBdI Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the MuBdI
...
to the MuBdI GitHub release page.
...
- [Building MuBdI Core](#building-mubdi-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/mubdi-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/mubdi-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/mubdi-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/mubdi-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/mubdi-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/mubdi-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/mubdi-project/gitian.sigs/) repository:
...
```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/mubdi-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/mubdi-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/mubdi-osx-*-build.assert
```
