UNIX BUILD NOTES
====================
Some notes on how to build MuBdI Core in Unix.

Note
---------------------
Always use absolute paths to configure and compile MuBdI Core and the dependencies,
...
This will build mubdi-qt as well, if the dependencies are met.
...
memory available when compiling MuBdI Core. On systems with less, gcc can be
 ...
 **For Ubuntu only:** db4.8 packages are available [here](https://launchpad.net/~mubdi/+archive/mubdi).
 You can add the repository using the following command:

    sudo apt-get install software-properties-common
    sudo add-apt-repository ppa:mubdi/mubdi
    sudo apt-get update
...
To build MuBdI Core without wallet, see [*Disable-wallet mode*](/doc/build-unix.md#disable-wallet-mode)
...
If you want to build mubdi-qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a mubdi-qt executable will be
built by default.
...
Notes
-----
The release is built with GCC and then "strip mubdid" to strip the debug
...
Security
--------
To help make your MuBdI Core installation more secure by making certain attacks impossible to
...
* _Non-executable Stack_: If the stack is executable then trivial stack-based buffer overflow exploits are possible if
    vulnerable buffers are found. By default, MuBdI Core should be built with a non-executable stack
