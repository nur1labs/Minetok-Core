...
- [Development guidelines](#development-guidelines)
    - [General MuBdI Core](#general-mubdi-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
MuBdI Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play MUBs (tMUB)" on the test network, if you
...
MuBdI Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/mubdi-project/mubdi/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_mubdi
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_mubdi --log_level=test_suite
$ valgrind -v --leak-check=full src/mubdid -printtoconsole
```
...
# A coverage report will now be accessible at `./test_mubdi.coverage/index.html`.
```

**Sanitizers**
MuBdI can be compiled with various "sanitizers" enabled, which add
...
- MuBdIMiner : Generates MUBs (if wallet is enabled).
...
However, in open source software such as MuBdI Core, where everyone uses
...
pay attention to for reviewers of MuBdI Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: MuBdI Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
- Use include guards to avoid the problem of double inclusion. The header file
  `foo/bar.h` should use the include guard identifier `MUBDI_FOO_BAR_H`, e.g.

```c++
#ifndef MUBDI_FOO_BAR_H
#define MUBDI_FOO_BAR_H
...
#endif /* MUBDI_FOO_BAR_H */
```
...
Some of these are maintained by active developers of MuBdI Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against MuBdI Core so that they can be integrated
...
file descriptors it will cause problems with MuBdI's `select()` loop, because
...
compatibility between nodes. This happened in MuBdI 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to MuBdI's
...
Commit [`bb81e173`](https://github.com/mubdi/mubdi/commit/bb81e173) is an example of a scripted-diff.
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:MuBdI-Project/MuBdI.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `mubdi-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `mubdi-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of MuBdI Core, the wallet was always
