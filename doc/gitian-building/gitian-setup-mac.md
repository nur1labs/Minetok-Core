...
The build script we'll be using is contained in the MuBdI github repository ([contrib/gitian-build.py](https://github.com/mubdi-project/mubdi/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the MuBdI repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/nur1labs/mubdi/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, mubdi-detached-sigs, and mubdi GitHub repos.
