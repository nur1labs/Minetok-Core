...
bool CMasternodePaymentDB::Write(const CMasternodePayments& objToSave)
{
    ...
    LogPrint(AILog::MASTERNODE,"Written info to mnpayments.dat  %dms\n", GetTimeMillis() - nStart);
...
CMasternodePaymentDB::ReadResult CMasternodePaymentDB::Read(CMasternodePayments& objToLoad)
{
    ...
    LogPrint(AILog::MASTERNODE,"Loaded info from mnpayments.dat (dbversion=%d) %dms\n", version, GetTimeMillis() - nStart);
    LogPrint(AILog::MASTERNODE,"  %s\n", objToLoad.ToString());
...
void DumpMasternodePayments()
{
    ...
    LogPrint(AILog::MASTERNODE,"Writing info to mnpayments.dat...\n");
    paymentdb.Write(masternodePayments);

    LogPrint(AILog::MASTERNODE,"Budget dump finished  %dms\n", GetTimeMillis() - nStart);
}
...
bool IsBlockPayeeValid(const CBlock& block, const CBlockIndex* pindexPrev)
{
    ...
    if (!masternodeSync.IsSynced()) { //there is no budget data to use to check anything -- find the longest chain
        LogPrint(AILog::MASTERNODE, "Client not synced, skipping block payee checks\n");
        return true;
    }

    const bool fPayCoinstake = Params().GetConsensus().NetworkUpgradeActive(nBlockHeight, Consensus::UPGRADE_POS) &&
                               !Params().GetConsensus().NetworkUpgradeActive(nBlockHeight, Consensus::UPGRADE_V6);
    ...
    if (sporkManager.IsSporkActive(SPORK_5_ENABLE_SUPERBLOCKS)) {
        if (g_budgetman.IsBudgetPaymentBlock(nBlockHeight)) {
            ...
            if (transactionStatus == TrxValidationStatus::InValid) {
                LogPrint(AILog::MASTERNODE,"Invalid budget payment detected %s\n", txNew.ToString().c_str());
                if (sporkManager.IsSporkActive(SPORK_4_MASTERNODE_BUDGET_ENFORCEMENT))
                    return false;

                LogPrint(AILog::MASTERNODE,"Budget enforcement is disabled, accepting block\n");
            }
        }
    }
    ...
    if (masternodePayments.IsTransactionValid(txNew, pindexPrev))
        return true;
    LogPrint(AILog::MASTERNODE,"Invalid mn payment detected %s\n", txNew.ToString().c_str());

    if (sporkManager.IsSporkActive(SPORK_3_MASTERNODE_PAYMENT_ENFORCEMENT))
        return false;
    LogPrint(AILog::MASTERNODE,"Masternode payment enforcement is disabled, accepting block\n");
    return true;
}
...
{
    if (!sporkManager.IsSporkActive(SPORK_5_ENABLE_SUPERBLOCKS) ||           // if superblocks are not enabled
...
{
    if (sporkManager.IsSporkActive(SPORK_5_ENABLE_SUPERBLOCKS) && g_budgetman.IsBudgetPaymentBlock(nBlockHeight)) {
...
bool CMasternodePayments::GetLegacyMasternodeTxOut(int nHeight, std::vector<CTxOut>& voutMasternodePaymentsRet) const
{
    ...
    if (!GetBlockPayee(nHeight, payee)) {
        ...
        if (winningNode) {
            ...
            LogPrint(AILog::MASTERNODE,"CreateNewBlock: Failed to detect masternode to pay\n");
            return false;
        }
    }
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txCoinbase, CMutableTransaction& txCoinstake, const CBlockIndex* pindexPrev, bool fProofOfStake) const
{
    ...
    // Starting from MuBdI v6.0 masternode and budgets are paid in the coinbase tx (block v10)
    const int nHeight = pindexPrev->nHeight + 1;
    bool fPayCoinstake = fProofOfStake && !Params().GetConsensus().NetworkUpgradeActive(nHeight, Consensus::UPGRADE_V6);
    ...
    for (const CTxOut& mnOut: vecMnOuts) {
        ...
        LogPrint(AILog::MASTERNODE,"Masternode payment of %s to %s\n", FormatMoney(mnOut.nValue), EncodeDestination(payeeDest));
    }
...
void CMasternodePayments::ProcessMessageMasternodePayments(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == NetMsgType::GETMNWINNERS) { //Masternode Payments Request Sync
        ...
        LogPrint(AILog::MASTERNODE, "mnget - Sent Masternode winners to peer %i\n", pfrom->GetId());
...
bool CMasternodePayments::ProcessMNWinner(CMasternodePaymentWinner& winner, CNode* pfrom, CValidationState& state)
{
    ...
    if (mapMasternodePayeeVotes.count(winner.GetHash())) {
        LogPrint(AILog::MASTERNODE, "mnw - Already seen - %s bestHeight %d\n", winner.GetHash().ToString().c_str(), nHeight);
    ...
    if (winner.nBlockHeight < nFirstBlock || winner.nBlockHeight > nHeight + 20) {
        LogPrint(AILog::MASTERNODE, "mnw - winner out of range - FirstBlock %d Height %d bestHeight %d\n", nFirstBlock, winner.nBlockHeight, nHeight);
    ...
    if (winner.nMessVersion != MessageVersion::MESS_VER_HASH) {
        LogPrint(AILog::MASTERNODE, "mnw - rejecting old message version %d\n", winner.nMessVersion);
    ...
    // Check signature
    bool is_valid_sig = winner.CheckSignature(pmn->pubKeyMasternode.GetID());

    if (!is_valid_sig) {
        LogPrint(AILog::MASTERNODE, "%s : mnw - invalid signature for %s masternode: %s\n",
                __func__, ("legacy"), winner.vinMasternode.prevout.hash.ToString());
        if (pfrom) {
            LOCK(cs_main);
            Misbehaving(pfrom->GetId(), 20);
        }
...
void CMasternodePayments::AddWinningMasternode(CMasternodePaymentWinner& winnerIn)
{
    ...
    LogPrint(AILog::MASTERNODE, "mnw - Adding winner %s for block %d\n", EncodeDestination(addr), winnerIn.nBlockHeight);
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
    ...
    LogPrint(AILog::MASTERNODE,"CMasternodePayments::IsTransactionValid - Missing required payment of %s to %s\n", FormatMoney(requiredMasternodePayment).c_str(), strPayeesPossible.c_str());
    return false;
}
...
void CMasternodePayments::CleanPaymentList(int mnCount, int nHeight)
{
    ...
    while (it != mapMasternodePayeeVotes.end()) {
        ...
        if (nHeight - winner.nBlockHeight > nLimit) {
            LogPrint(AILog::MASTERNODE, "CMasternodePayments::CleanPaymentList - Removing old Masternode payment - block %d\n", winner.nBlockHeight);
...
bool IsCoinbaseValueValid(const CTransactionRef& tx, CAmount nBudgetAmt, CValidationState& _state)
{
    ...
    if (masternodeSync.IsSynced()) {
        ...
        if (nBudgetAmt > 0) {
            ...
            bool sporkEnforced = sporkManager.IsSporkActive(SPORK_3_MASTERNODE_PAYMENT_ENFORCEMENT);
