...
void OnRPCStopped()
{
    ...
    LogPrint(AILog::RPC, "RPC stopped.\n");
}
...
std::string HelpMessage(HelpMessageMode mode)
{
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf("Specify pid file (default: %s)", MUBDI_PID_FILENAME));
#endif
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf("Disable all MuBdI specific functionality (Masternodes, Budgeting) (0-1, default: %u)", 0));
    ...
    strUsage += HelpMessageOpt("-mnconf=<file>", strprintf("Specify masternode configuration file (default: %s)", MUBDI_MASTERNODE_CONF_FILENAME));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf("Set external address:port to get to this masternode (example: %s)", "128.127.106.235:40031"));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2010-%i %s"), COPYRIGHT_YEAR, PACKAGE_NAME)) + "\n" +
           "\n" +
...
bool AppInitMain()
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", MUBDI_CONF_FILENAME)).string());
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile because if MuBdI is started in the future "
                  "from a different location. It will be unable to locate the current data files. There could "
                  "also be data loss if MuBdI is started while in a temporary directory.\n",
...
// ********************************************************* Step 8: Backup and Load wallet
    ...
    if (!vpwallets.empty())
        GenerateMuBdIs(gArgs.GetBoolArg("-gen", DEFAULT_GENERATE), vpwallets[0], gArgs.GetArg("-genproclimit", DEFAULT_GENERATE_PROCLIMIT));
#endif
