...
int CAddrInfo::GetTriedBucket(const uint256& nKey, const std::vector<bool> &asmap) const
{
    ...
    LogPrint(AILog::NET, "IP %s mapped to AS%i belongs to tried bucket %i\n", ToStringIP(), mapped_as, tried_bucket);
    return tried_bucket;
}
...
int CAddrInfo::GetNewBucket(const uint256& nKey, const CNetAddr& src, const std::vector<bool> &asmap) const
{
    ...
    uint32_t mapped_as = GetMappedAS(asmap);
    LogPrint(AILog::NET, "IP %s mapped to AS%i belongs to new bucket %i\n", ToStringIP(), mapped_as, new_bucket);
    return new_bucket;
}
...
void CAddrMan::Good_(const CService& addr, bool test_before_evict, int64_t nTime)
{
    ...
    if (test_before_evict && (vvTried[tried_bucket][tried_bucket_pos] != -1)) {
        ...
        LogPrint(AILog::ADDRMAN, "Collision inserting element into tried table (%s), moving %s to m_tried_collisions=%d\n", colliding_entry != mapInfo.end() ? colliding_entry->second.ToString() : "", addr.ToString(), m_tried_collisions.size());
        if (m_tried_collisions.size() < ADDRMAN_SET_TRIED_COLLISION_SIZE) {
            m_tried_collisions.insert(nId);
        }
    } else {
        LogPrint(AILog::ADDRMAN, "Moving %s to tried\n", addr.ToString());
            ...
            if (!info_new.IsValid()) { // id_new may no longer map to a valid address
                ...
                if (GetAdjustedTime() - info_old.nLastSuccess < ADDRMAN_REPLACEMENT_HOURS*(60*60)) {
                    ...
                    if (GetAdjustedTime() - info_old.nLastTry > 60) {
                        LogPrint(AILog::ADDRMAN, "Replacing %s with %s in tried table\n", info_old.ToString(), info_new.ToString());
                    ...
                    LogPrint(AILog::ADDRMAN, "Unable to test; replacing %s with %s in tried table anyway\n", info_old.ToString(), info_new.ToString());
                    Good_(info_new, false, GetAdjustedTime());
                    erase_collision = true;
                }
