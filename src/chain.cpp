...
int64_t CBlockIndex::MinPastBlockTime() const
{
    ...
    // so we allow the nBlockTimeProtocolV2 (MuBdI v4) to be at most (180-14) seconds earlier than previous block
    if (nHeight + 1 == consensus.vUpgrades[Consensus::UPGRADE_V4].nActivationHeight)
...
void CBlockIndex::SetNewStakeModifier(const uint256& prevoutId)
{
    ...
    if (!Params().GetConsensus().NetworkUpgradeActive(nHeight, Consensus::UPGRADE_V3)) return;
...
{
    if (vStakeModifier.empty() || Params().GetConsensus().NetworkUpgradeActive(nHeight, Consensus::UPGRADE_V3))
        return 0;
