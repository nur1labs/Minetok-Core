...
void CBudgetManager::CheckOrphanVotes()
{
    ...
    LogPrint(AILog::MNBUDGET,"%s: Done\n", __func__);
}
...
uint256 CBudgetManager::SubmitFinalBudget()
{
    ...
    if (nSubmittedHeight >= nBlockStart){
        LogPrint(AILog::MNBUDGET,"%s: nSubmittedHeight(=%ld) < nBlockStart(=%ld) condition not fulfilled.\n",
    ...
    if (nBlockStart - nCurrentHeight > finalizationWindow) {
        LogPrint(AILog::MNBUDGET,"%s: Too early for finalization. Current block is %ld, next Superblock is %ld.\n", __func__, nCurrentHeight, nBlockStart);
        LogPrint(AILog::MNBUDGET,"%s: First possible block for finalization: %ld. Last possible block for finalization: %ld. "
    ...
    if (vecTxBudgetPayments.size() < 1) {
        LogPrint(AILog::MNBUDGET,"%s: Found No Proposals For Period\n", __func__);
        return UINT256_ZERO;
    }
    ...
    if (HaveFinalizedBudget(budgetHash)) {
        LogPrint(AILog::MNBUDGET,"%s: Budget already exists - %s\n", __func__, budgetHash.ToString());
    ...
    if (!mapUnconfirmedFeeTx.count(budgetHash)) {
        ...
        if (vpwallets.empty() || !vpwallets[0]) {
            LogPrint(AILog::MNBUDGET,"%s: Wallet not found\n", __func__);
            return UINT256_ZERO;
        }
        CReserveKey keyChange(vpwallets[0]);
        if (!vpwallets[0]->CreateBudgetFeeTX(wtx, budgetHash, keyChange, true)) {
            LogPrint(AILog::MNBUDGET,"%s: Can't make collateral transaction\n", __func__);
            return UINT256_ZERO;
        }
        ...
        if (res.status == CWallet::CommitStatus::OK) {
            ...
            LogPrint(AILog::MNBUDGET,"%s: Collateral sent. txid: %s\n", __func__, collateraltxid.ToString());
            return budgetHash;
        }
        return UINT256_ZERO;
    }
    ...
    fb.Relay();
    nSubmittedHeight = nCurrentHeight;
    LogPrint(AILog::MNBUDGET,"%s: Done! %s\n", __func__, budgetHash.ToString());
    return budgetHash;
}
...
std::string CBudgetManager::GetFinalizedBudgetStatus(const uint256& nHash) const
{
    ...
    for (int nBlockHeight = nBlockStart; nBlockHeight <= nBlockEnd; nBlockHeight++) {
        ...
        if (!fb.GetBudgetPaymentByBlock(nBlockHeight, budgetPayment)) {
            LogPrint(AILog::MNBUDGET,"%s: Couldn't find budget payment for block %lld\n", __func__, nBlockHeight);
            continue;
        }
...
bool CBudgetManager::AddFinalizedBudget(CFinalizedBudget& finalizedBudget, CNode* pfrom)
{
    ...
    if (!CheckCollateral(feeTxId, nHash, strError, finalizedBudget.nTime, nCurrentHeight, true)) {
        LogPrint(AILog::MNBUDGET,"%s: invalid finalized budget (%s) collateral id=%s - %s\n",
    ...
    if (!finalizedBudget.CheckProposals(mapWinningProposals)) {
        ...
        LogPrint(AILog::MNBUDGET,"%s: Budget finalization does not match with winning proposals\n", __func__);
    ...
    LogPrint(AILog::MNBUDGET,"%s: finalized budget %s [%s (%s)] added\n",
...
bool CBudgetManager::AddProposal(CBudgetProposal& budgetProposal)
{
    ...
    if (!CheckCollateral(feeTxId, nHash, strError, budgetProposal.nTime, nCurrentHeight, false)) {
        LogPrint(AILog::MNBUDGET,"%s: invalid budget proposal (%s) collateral id=%s - %s\n",
                __func__, nHash.ToString(), feeTxId.ToString(), strError);
    ...
    LogPrint(AILog::MNBUDGET,"%s: budget proposal %s [%s] added\n", __func__, nHash.ToString(), budgetProposal.GetName());
...
void CBudgetManager::CheckAndRemove()
{
    ...
    {
        LOCK(cs_proposals);
        LogPrint(AILog::MNBUDGET, "%s: mapProposals cleanup - size before: %d\n", __func__, mapProposals.size());
        for (auto& it: mapProposals) {
            ...
            if (!pbudgetProposal->UpdateValid(nCurrentHeight)) {
                ...
                mapFeeTxToProposal.erase(pbudgetProposal->GetFeeTXHash());
                 ...
                 LogPrint(AILog::MNBUDGET,"%s: Found valid budget proposal: %s %s\n", __func__,
        ...
        LogPrint(AILog::MNBUDGET, "%s: mapProposals cleanup - size after: %d\n", __func__, mapProposals.size());
    }

    // Then check finalized budgets
    {
        LOCK(cs_budgets);
        LogPrint(AILog::MNBUDGET, "%s: mapFinalizedBudgets cleanup - size before: %d\n", __func__, mapFinalizedBudgets.size());
        for (auto& it: mapFinalizedBudgets) {
            ...
            if (!pfinalizedBudget->UpdateValid(nCurrentHeight)) {
                ...
                LogPrint(AILog::MNBUDGET,"%s: Found valid finalized budget: %s %s\n", __func__,
        ...
        LogPrint(AILog::MNBUDGET, "%s: mapFinalizedBudgets cleanup - size after: %d\n", __func__, mapFinalizedBudgets.size());
    }
...
bool CBudgetManager::FillBlockPayee(CMutableTransaction& txCoinbase, CMutableTransaction& txCoinstake, const int nHeight, bool fProofOfStake) const
{
    ...
    // Starting from MuBdI v6.0 masternode and budgets are paid in the coinbase tx of PoS blocks
    const bool fPayCoinstake = fProofOfStake &&
                               !Params().GetConsensus().NetworkUpgradeActive(nHeight, Consensus::UPGRADE_V6);
    ...
    LogPrint(AILog::MNBUDGET,"%s: Budget payment to %s for %lld\n", __func__, EncodeDestination(address), nAmount);
    return true;
}
...
void CBudgetManager::VoteOnFinalizedBudgets()
{
    ...
    if (!fMasterNode) {
        LogPrint(AILog::MNBUDGET,"%s: Not a masternode\n", __func__);
        return;
    }
    ...
    if (GetRandInt(4) != 0) {
        LogPrint(AILog::MNBUDGET,"%s: waiting\n", __func__);
        return;
    }
    ...
    std::vector<CBudgetProposal> vBudget = GetBudget();
    if (vBudget.empty()) {
        LogPrint(AILog::MNBUDGET,"%s: No proposal can be finalized\n", __func__);
        return;
    }
    ...
    for (const uint256& budgetHash: vBudgetHashes) {
        ...
        LogPrint(AILog::MNBUDGET, "%s: new finalized budget vote signed: %s\n", __func__, vote.GetHash().ToString());
...
bool CBudgetManager::IsBudgetPaymentBlock(int nBlockHeight, int& nCountThreshold) const
{
    ...
    LogPrint(AILog::MNBUDGET,"%s: nHighestCount: %lli, 5%% of Masternodes: %lli.\n",
...
TrxValidationStatus CBudgetManager::IsTransactionValid(const CTransaction& txNew, const uint256& nBlockHash, int nBlockHeight) const
{
    ...
    bool fThreshold = false;
    {
        ...
        if (highestVotesBudget) {
            ...
            // tx not valid
            LogPrint(AILog::MNBUDGET, "%s: ignoring budget. Out of range or tx not valid.\n", __func__);
        }
    }
...
std::vector<CBudgetProposal> CBudgetManager::GetBudget()
{
    ...
    for (CBudgetProposal* pbudgetProposal: vProposalsOrdered) {
        LogPrint(AILog::MNBUDGET,"%s: Processing Budget %s\n", __func__, pbudgetProposal->GetName());
        //prop start/end should be inside this period
        if (pbudgetProposal->IsPassing(nBlockStart, nBlockEnd, mnCount)) {
            LogPrint(AILog::MNBUDGET,"%s:  -   Check 1 passed: valid=%d | %ld <= %ld | %ld >= %ld | Yeas=%d Nays=%d Count=%d | established=%d\n",
            ...
            if (pbudgetProposal->GetAmount() + nBudgetAllocated <= nTotalBudget) {
                ...
                LogPrint(AILog::MNBUDGET,"%s:  -     Check 2 passed: Budget added\n", __func__);
            } else {
                pbudgetProposal->SetAllotted(0);
                LogPrint(AILog::MNBUDGET,"%s:  -     Check 2 failed: no amount allotted\n", __func__);
            }

        } else {
            LogPrint(AILog::MNBUDGET,"%s:  -   Check 1 failed: valid=%d | %ld <= %ld | %ld >= %ld | Yeas=%d Nays=%d Count=%d | established=%d\n",
                    __func__, pbudgetProposal->IsValid(), pbudgetProposal->GetBlockStart(), nBlockStart, pbudgetProposal->GetBlockEnd(),
    ...
    while (it != mapFinalizedBudgets.end()) {
        ...
        if (nBlockHeight >= pfinalizedBudget->GetBlockStart() && nBlockHeight <= pfinalizedBudget->GetBlockEnd()) {
            ...
            if (pfinalizedBudget->GetBudgetPaymentByBlock(nBlockHeight, payment)) {
                ...
                LogPrint(AILog::MNBUDGET,"%s:  Couldn't find budget payment for block %d\n", __func__, nBlockHeight);
            }
        }
...
{
    AssertLockHeld(cs_proposals);
    LogPrint(AILog::MNBUDGET, "Cleaning proposal votes for %s. Before: YES=%d, NO=%d\n",
    ...
    LogPrint(AILog::MNBUDGET, "Cleaned proposal votes for %s. After: YES=%d, NO=%d\n",
            prop->GetName(), prop->GetYeas(), prop->GetNays());
}
...
{
    AssertLockHeld(cs_budgets);
    LogPrint(AILog::MNBUDGET, "Cleaning finalized budget votes for [%s (%s)]. Before: %d\n",
    ...
    LogPrint(AILog::MNBUDGET, "Cleaned finalized budget votes for [%s (%s)]. After: %d\n",
            fbud->GetName(), fbud->GetProposalsStr(), fbud->GetVoteCount());
}
...
void CBudgetManager::NewBlock()
{
    ...
    if (masternodeSync.IsSynced()) {
        LogPrint(AILog::MNBUDGET,"%s:  incremental sync started\n", __func__);
    ...
    LogPrint(AILog::MNBUDGET,"%s:  askedForSourceProposalOrBudget cleanup - size: %d\n", __func__, askedForSourceProposalOrBudget.size());
    ...
    {
        LOCK(cs_proposals);
        LogPrint(AILog::MNBUDGET,"%s:  mapProposals cleanup - size: %d\n", __func__, mapProposals.size());
        for (auto& it: mapProposals) {
    ...
    {
        LOCK(cs_budgets);
        LogPrint(AILog::MNBUDGET,"%s:  mapFinalizedBudgets cleanup - size: %d\n", __func__, mapFinalizedBudgets.size());
        for (auto& it: mapFinalizedBudgets) {
    ...
    LogPrint(AILog::MNBUDGET,"%s:  PASSED\n", __func__);
}
...
int CBudgetManager::ProcessBudgetVoteSync(const uint256& nProp, CNode* pfrom)
{
    if (nProp.IsNull()) {
        ...
        if (!(pfrom->addr.IsRFC1918() || pfrom->addr.IsLocal())) {
            ...
            if (itLastRequest != mAskedUsForBudgetSync.end() && GetTime() < (*itLastRequest).second) {
                LogPrint(AILog::MASTERNODE, "budgetsync - peer %i already asked for budget sync\n", pfrom->GetId());
    ...
    Sync(pfrom, nProp);
    LogPrint(AILog::MNBUDGET, "mnvs - Sent Masternode votes to peer %i\n", pfrom->GetId());
    return 0;
}
...
int CBudgetManager::ProcessProposal(CBudgetProposal& proposal)
{
    ...
    LogPrint(AILog::MNBUDGET, "mprop (new) %s\n", nHash.ToString());
...
bool CBudgetManager::ProcessProposalVote(CBudgetVote& vote, CNode* pfrom, CValidationState& state)
{
    ...
    masternodeSync.AddedBudgetItem(voteID);
    LogPrint(AILog::MNBUDGET, "mvote - new vote (%s) for proposal %s from dmn %s\n",
...
int CBudgetManager::ProcessFinalizedBudget(CFinalizedBudget& finalbudget, CNode* pfrom)
{
    ...
    LogPrint(AILog::MNBUDGET, "fbs (new) %s\n", nHash.ToString());
...
bool CBudgetManager::ProcessFinalizedBudgetVote(CFinalizedBudgetVote& vote, CNode* pfrom, CValidationState& state)
{
    ...
    if (masternodeSync.IsSynced()) vote.Relay();
    masternodeSync.AddedBudgetItem(voteID);
    LogPrint(AILog::MNBUDGET, "fbvote - new vote (%s) for budget %s from mn %s\n",
...
void CBudgetManager::Sync(CNode* pfrom, const uint256& nProp, bool fPartial)
{
    ...
    LogPrint(AILog::MNBUDGET, "%s: sent %d items\n", __func__, nInvCount);
...
bool CBudgetManager::UpdateProposal(const CBudgetVote& vote, CNode* pfrom, std::string& strError)
{
    ...
    if (!mapProposals.count(nProposalHash)) {
        if (pfrom) {
            ...
            LogPrint(AILog::MNBUDGET,"%s: Unknown proposal %d, asking for source proposal\n", __func__, nProposalHash.ToString());
...
bool CBudgetManager::UpdateFinalizedBudget(CFinalizedBudgetVote& vote, CNode* pfrom, std::string& strError)
{
    ...
    if (!mapFinalizedBudgets.count(nBudgetHash)) {
        if (pfrom) {
            ...
            LogPrint(AILog::MNBUDGET,"%s: Unknown Finalized Proposal %s, asking for source budget\n", __func__, nBudgetHash.ToString());
    ...
    LogPrint(AILog::MNBUDGET,"%s: Finalized Proposal %s added\n", __func__, nBudgetHash.ToString());
...
bool CheckCollateral(const uint256& nTxCollateralHash, const uint256& nExpectedHash, std::string& strError, int64_t& nTime, int nCurrentHeight, bool fBudgetFinalization)
{
    ...
    for (const CTxOut &o : txCollateral->vout) {
        ...
        if (fBudgetFinalization) {
            ...
            // Note: there are still old valid budgets out there, but the check for the new 5 MUB finalization collateral
            //       will also cover the old 50 MUB finalization collateral.
            LogPrint(AILog::MNBUDGET, "Final Budget: o.scriptPubKey(%s) == findScript(%s) ?\n", HexStr(o.scriptPubKey), HexStr(findScript));
            if (o.scriptPubKey == findScript) {
                LogPrint(AILog::MNBUDGET, "Final Budget: o.nValue(%ld) >= BUDGET_FEE_TX(%ld) ?\n", o.nValue, BUDGET_FEE_TX);
            ...
            LogPrint(AILog::MNBUDGET, "Normal Budget: o.scriptPubKey(%s) == findScript(%s) ?\n", HexStr(o.scriptPubKey), HexStr(findScript));
            if (o.scriptPubKey == findScript) {
                LogPrint(AILog::MNBUDGET, "Normal Budget: o.nValue(%ld) >= PROPOSAL_FEE_TX(%ld) ?\n", o.nValue, PROPOSAL_FEE_TX);
