...
bool CBudgetDB::Write(const CBudgetManager& objToSave)
{
    ...
    LogPrint(AILog::MNBUDGET,"Written info to budget.dat  %dms\n", GetTimeMillis() - nStart);
...
CBudgetDB::ReadResult CBudgetDB::Read(CBudgetManager& objToLoad, bool fDryRun)
{
    ...
    LogPrint(AILog::MNBUDGET,"Loaded info from budget.dat (dbversion=%d) %dms\n", version, GetTimeMillis() - nStart);
    LogPrint(AILog::MNBUDGET,"%s\n", objToLoad.ToString());
    if (!fDryRun) {
        LogPrint(AILog::MNBUDGET,"Budget manager - cleaning....\n");
        objToLoad.CheckAndRemove();
        LogPrint(AILog::MNBUDGET,"Budget manager - result: %s\n", objToLoad.ToString());
    }
...
void DumpBudgets(CBudgetManager& budgetman)
{
    ...
    CBudgetDB budgetdb;
    LogPrint(AILog::MNBUDGET,"Writing info to budget.dat...\n");
    budgetdb.Write(budgetman);

    LogPrint(AILog::MNBUDGET,"Budget dump finished  %dms\n", GetTimeMillis() - nStart);
}
