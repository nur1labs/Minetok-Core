        ...
        if (ch == ' ') {
            ...
            if (self->message.code >= 600) {
                ...
                if (!self->reply_handlers.empty()) {
                    ...
                    LogPrint(AILog::TOR, "tor: Received unexpected sync reply %i\n", self->message.code);
                }
            }
            self->message.Clear();
        }
...
void TorControlConnection::eventcb(struct bufferevent *bev, short what, void *ctx)
{
    ...
    if (what & BEV_EVENT_CONNECTED) {
        LogPrint(AILog::TOR, "tor: Successfully connected!\n");
        self->connected(*self);
    } else if (what & (BEV_EVENT_EOF|BEV_EVENT_ERROR)) {
        if (what & BEV_EVENT_ERROR) {
            LogPrint(AILog::TOR, "tor: Error connecting to Tor control socket\n");
        } else {
            LogPrint(AILog::TOR, "tor: End of stream\n");
        }
        self->Disconnect();
    ...
    reconnect_timeout(RECONNECT_TIMEOUT_START)
{
    ...
    if (pkf.first) {
        LogPrint(AILog::TOR, "tor: Reading cached private key from %s\n", GetPrivateKeyFile().string());
...
{
    if (reply.code == 250) {
        LogPrint(AILog::TOR, "tor: ADD_ONION successful\n");
        ...
        if (WriteBinaryFile(GetPrivateKeyFile(), private_key)) {
            LogPrint(AILog::TOR, "tor: Cached service private key to %s\n", GetPrivateKeyFile().string());
        } else {
...
{
    if (reply.code == 250) {
        LogPrint(AILog::TOR, "tor: Authentication successful\n");
...
{
    if (reply.code == 250) {
        LogPrint(AILog::TOR, "tor: SAFECOOKIE authentication challenge successful\n");
        ...
        if (l.first == "AUTHCHALLENGE") {
            ...
            LogPrint(AILog::TOR, "tor: AUTHCHALLENGE ServerHash %s ServerNonce %s\n", HexStr(serverHash), HexStr(serverNonce));
    ...
    if (reply.code == 250) {
        ...
        for (const std::string &s : reply.lines) {
            ...
            if (l.first == "AUTH") {
                ...
                if ((i = m.find("Tor")) != m.end()) {
                    LogPrint(AILog::TOR, "tor: Connected to Tor version %s\n", i->second);
                }
            }
        }
        for (const std::string &s : methods) {
            LogPrint(AILog::TOR, "tor: Supported authentication method: %s\n", s);
        }
        ...
        if (!torpassword.empty()) {
            if (methods.count("HASHEDPASSWORD")) {
                LogPrint(AILog::TOR, "tor: Using HASHEDPASSWORD authentication\n");
            ...
            LogPrint(AILog::TOR, "tor: Using NULL authentication\n");
            _conn.Command("AUTHENTICATE", std::bind(&TorController::auth_cb, this, std::placeholders::_1, std::placeholders::_2));
        } else if (methods.count("SAFECOOKIE")) {
            // Cookie: hexdump -e '32/1 "%02x""\n"'  ~/.tor/control_auth_cookie
            LogPrint(AILog::TOR, "tor: Using SAFECOOKIE authentication, reading cookie authentication from %s\n", cookiefile);
...
void TorController::disconnected_cb(TorControlConnection& _conn)
{
    ...
    LogPrint(AILog::TOR, "tor: Not connected to Tor control port %s, trying to reconnect\n", target);
