...
std::vector<CSporkDef> sporkDefs = {
    MAKE_SPORK_DEF(SPORK_3_MASTERNODE_PAYMENT_ENFORCEMENT,  4070908800ULL), // OFF
    MAKE_SPORK_DEF(SPORK_4_MASTERNODE_BUDGET_ENFORCEMENT,   4070908800ULL), // OFF
    MAKE_SPORK_DEF(SPORK_5_ENABLE_SUPERBLOCKS,              4070908800ULL), // OFF
    MAKE_SPORK_DEF(SPORK_6_NEW_PROTOCOL_ENFORCEMENT,        4070908800ULL), // OFF
    MAKE_SPORK_DEF(SPORK_7_NEW_PROTOCOL_ENFORCEMENT_2,      4070908800ULL), // OFF
    MAKE_SPORK_DEF(SPORK_8_COLDSTAKING_ENFORCEMENT,         4070908800ULL), // OFF
    MAKE_SPORK_DEF(SPORK_9_COLDSTAKING_MAINTENANCE,         4070908800ULL), // OFF
    MAKE_SPORK_DEF(SPORK_10_BLOCKSTREAM_ENCHANCEMENT,       4070908800ULL), // OFF
    MAKE_SPORK_DEF(SPORK_11_GENESIS_ENCHANCEMENT,           4070908800ULL), // OFF
    MAKE_SPORK_DEF(SPORK_12_LEGACY_MNS_MAX_HEIGHT,          4070908800ULL), // OFF
};
...
void CSporkManager::LoadSporksFromDB()
{
    for (const auto& sporkDef : sporkDefs) {
        ...
        if (spork.nSporkID == SPORK_7_NEW_PROTOCOL_ENFORCEMENT_2) {
...
int CSporkManager::ProcessSporkMsg(CSporkMessage& spork)
{
    ...
    if (spork.nTimeSigned > GetAdjustedTime() + 2 * 60 * 60) {
        LogPrint(AILog::SPORKS, "%s : ERROR: too far into the future\n", __func__);
        return 100;
    }

    // reject old signature version
    if (spork.nMessVersion != MessageVersion::MESS_VER_HASH) {
        LogPrint(AILog::SPORKS, "%s : nMessVersion=%d not accepted anymore\n", __func__, spork.nMessVersion);
        return 0;
    }
    ...
    std::string strStatus;
    {
        ...
        if (mapSporksActive.count(spork.nSporkID)) {
            ...
            if (mapSporksActive[spork.nSporkID].nTimeSigned >= spork.nTimeSigned) {
                ...
                LogPrint(AILog::SPORKS, "%s : spork %d (%s) in memory is more recent: %d >= %d\n", __func__,
    ...
    if (!fValidSig) {
        LogPrint(AILog::SPORKS, "%s : Invalid Signature\n", __func__);
        return 100;
    }
...
std::string CSporkManager::GetSporkNameByID(SporkId nSporkID)
{
    auto it = sporkDefsById.find(nSporkID);
    if (it == sporkDefsById.end()) {
        LogPrint(AILog::SPORKS, "%s : Unknown Spork ID %d\n", __func__, nSporkID);
        return "Unknown";
    }
