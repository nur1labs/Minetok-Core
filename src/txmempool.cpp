...
CTxMemPoolEntry::CTxMemPoolEntry(const CTransactionRef& _tx, const CAmount& _nFee,
     ...
     tx(MakeTransactionRef(_tx)), nFee(_nFee), nTime(_nTime), entryPriority(_entryPriority), entryHeight(_entryHeight), inChainInputValue(_inChainInputValue), 
...
bool CTxMemPool::addUnchecked(const uint256& hash, const CTxMemPoolEntry &entry, setEntries &setAncestors, bool validFeeEstimate)
{
    ...
    if (pos != mapDeltas.end()) {
        const std::pair<double, CAmount> &delta = pos->second;
        if (delta.second) {
            mapTx.modify(newit, update_fee_delta(delta.second));
        }
    }
...
void CTxMemPool::check(const CCoinsViewCache* pcoins) const
{
    ...
    LogPrint(AILog::MEMPOOL, "Checking mempool with %u transactions and %u inputs\n", (unsigned int)mapTx.size(), (unsigned int)mapNextTx.size());
...
void CTxMemPool::PrioritiseTransaction(const uint256& hash, double dPriorityDelta, const CAmount& nFeeDelta)
{
    {
        LOCK(cs);
        std::pair<double, CAmount> &delta = mapDeltas[hash];
        delta.first += dPriorityDelta;
        delta.second += nFeeDelta;
        txiter it = mapTx.find(hash);
        if (it != mapTx.end()) {
            mapTx.modify(it, update_fee_delta(delta.second));
...
void CTxMemPool::ApplyDelta(const uint256& hash, double& dPriorityDelta, CAmount& nFeeDelta) const
{
    LOCK(cs);
    std::map<uint256, std::pair<double, CAmount> >::const_iterator pos = mapDeltas.find(hash);
    if (pos == mapDeltas.end())
        return;
    const std::pair<double, CAmount> &delta = pos->second;
    dPriorityDelta += delta.first;
    nFeeDelta += delta.second;
}
...
void CTxMemPool::TrimToSize(size_t sizelimit, std::vector<COutPoint>* pvNoSpendsRemaining)
{
    ...
    if (maxFeeRateRemoved > CFeeRate(0))
        LogPrint(AILog::MEMPOOL, "Removed %u txn, rolling minimum fee bumped to %s\n", nTxnRemoved, maxFeeRateRemoved.ToString());
}
