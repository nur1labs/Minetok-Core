...
int64_t CWallet::GetKeyCreationTime(const CWDestination& dest)
{
    ...
    return transpDest ? GetKeyCreationTime(*transpDest) : 0;
}
...
bool CWallet::AddToWalletIfInvolvingMe(const CTransactionRef& ptx, const CWalletTx::Confirmation& confirm, bool fUpdate)
{
    const CTransaction& tx = *ptx;
    {
        ...
        if (fExisted || IsMine(ptx) || isFromMe) {
    ...
    const isminefilter& filter) const
{
    ...
    if (isFromMyTaddr) {// debit>0 means we signed/sent this transaction
        CAmount nValueOut = tx->GetValueOut(); // transasparent outputs
...
{
    const bool fIncludeColdStaking = !sporkManager.IsSporkActive(SPORK_8_COLDSTAKING_ENFORCEMENT) &&
    ...
    int nMinDepth)
{
    ...
    CTxMemPoolEntry entry(txRet, 0, 0, 0, false, 0, false, 0);
        ...
        bool stopOnNewBlock) const
{
    ...
    for (auto it = availableCoins->begin(); it != availableCoins->end();) {
        ...
        CMuBdIStake stakeInput(it->tx->tx->vout[it->i],
                             outPoint,
                             it->pindex);
    ...
    LogPrint(AILog::STAKING, "%s: attempted staking %d times\n", __func__, nAttempts);
...
CWallet* CWallet::CreateWalletFromFile(const std::string& name, const fs::path& path)
{
    ...
    if (gArgs.GetBoolArg("-upgradewallet", fFirstRun && !fLegacyWallet) ||
        ...
        if (prev_version <= FEATURE_PRE_MUBDI && walletInstance->IsLocked()) {
    ...
    if (fFirstRun) {
        if (!fLegacyWallet) {
            ...
            walletInstance->SetMaxVersion(FEATURE_PRE_MUBDI);
        }
