    ...
    stakeValue(stakeInput->GetValue())
{
    ...
    if (!Params().GetConsensus().NetworkUpgradeActive(pindexPrev->nHeight + 1, Consensus::UPGRADE_V3)) {
...
bool CStakeKernel::CheckKernelHash(bool fSkipLog) const
{
    ...
    if (!fSkipLog || res) {
        LogPrint(AILog::STAKING, "%s : Proof Of Stake:"
                            "\nstakeModifier=%s"
                            "\nnTimeBlockFrom=%d"
                            "\nssUniqueID=%s"
                            "\nnTimeTx=%d"
...
static bool LoadStakeInput(const CBlock& block, std::unique_ptr<CStakeInput>& stake, int nHeight)
{
    ...
    stake = std::unique_ptr<CStakeInput>(CMuBdIStake::NewMuBdIStake(txin, nHeight, block.nTime));
