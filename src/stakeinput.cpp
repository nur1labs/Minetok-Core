...
CMuBdIStake* CMuBdIStake::NewMuBdIStake(const CTxIn& txin, int nHeight, uint32_t nTime)
{
    ...
    if (!coin.IsSpent()) {
        ...
        // All good
        return new CMuBdIStake(coin.out, txin.prevout, pindexFrom);
    }
    ...
    // All good
    return new CMuBdIStake(txPrev->vout[txin.prevout.n], txin.prevout, pindexFrom);
}

bool CMuBdIStake::GetTxOutFrom(CTxOut& out) const
{
    out = outputFrom;
    return true;
}

CTxIn CMuBdIStake::GetTxIn() const
{
...
bool CMuBdIStake::CreateTxOuts(const CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal) const
{
...
const CBlockIndex* CMuBdIStake::GetIndexFrom() const
{
    // Sanity check, pindexFrom is set on the constructor.
    if (!pindexFrom) throw std::runtime_error("CMuBdIStake: uninitialized pindexFrom");
    return pindexFrom;
}
