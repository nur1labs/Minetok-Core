...
{
    IntrRecvError recvr;
    LogPrint(AILog::NET, "SOCKS5 connecting %s\n", strDest);
    if (strDest.size() > 255) {
    ...
    if (pchRet1[1] == SOCKS5Method::USER_PASS && auth) {
        ...
        LogPrint(AILog::PROXY, "SOCKS5 sending proxy authentication %s:%s\n", auth->username, auth->password);
        uint8_t pchRetA[2];
    ...
    LogPrint(AILog::NET, "SOCKS5 connected %s\n", strDest);
    return true;
}
...
bool ConnectSocketDirectly(const CService& addrConnect, const SOCKET& hSocket, int nTimeout)
{
    ...
    if (connect(hSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEINPROGRESS || nErr == WSAEWOULDBLOCK || nErr == WSAEINVAL) {
            ...
            if (nRet == 0) {
                LogPrint(AILog::NET, "connection to %s timeout\n", addrConnect.ToString());
                return false;
            }
            if (nRet == SOCKET_ERROR) {
