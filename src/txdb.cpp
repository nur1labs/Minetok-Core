                              ...
                              const uint256& hashBlock)
{
    ...
    for (CCoinsMap::iterator it = mapCoins.begin(); it != mapCoins.end();) {
        ...
        if (batch.SizeEstimate() > batch_size) {
            LogPrint(AILog::COINDB, "Writing partial batch of %.2f MiB\n", batch.SizeEstimate() * (1.0 / 1048576.0));
            db.WriteBatch(batch);
            batch.Clear();
            if (crash_simulate) {
    ...
    LogPrint(AILog::COINDB, "Writing final batch of %.2f MiB\n", batch.SizeEstimate() * (1.0 / 1048576.0));
    bool ret = db.WriteBatch(batch);
    LogPrint(AILog::COINDB, "Committed %u changed transaction outputs (out of %u) to coin database...\n", (unsigned int)changed, (unsigned int)count);
    return ret;
}
