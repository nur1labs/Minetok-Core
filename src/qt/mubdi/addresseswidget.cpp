...
void AddressesWidget::loadWalletModel()
{
    if (walletModel) {
        ...
        this->filter = new AddressFilterProxyModel(
                QStringList({AddressTableModel::Send, AddressTableModel::ColdStakingSend}),
                this);
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto MuBdIAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!Standard::IsValidDestination(MuBdIAdd)) {
        ...
        if (walletModel->updateAddressBookLabels(MuBdIAdd, label.toUtf8().constData(),
