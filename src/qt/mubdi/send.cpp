    ...
    coinIcon(new QPushButton())
{
    ...
    ui->btnUri->setSubTitleClassAndText("text-subtitle", tr("Parse a MuBdI URI"));
...
void SendWidget::refreshAmounts()
{
    ...
    if (coinControlDialog->coinControl->HasSelected()) {
        ...
        if (isTransparent) {
            totalAmount = balances.balance - walletModel->getLockedBalance() - total;
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent MUBs.
...
void SendWidget::run(int type)
{
    if (type == REQUEST_PREPARE_TX) {
        ...
        if (!isProcessing) {
            ...
            if ((result = prepareTransparent(ptrModelTx)
                        )) {
...
void SendWidget::setCoinControlPayAmounts()
{
    ...
    while (it.hasNext()) {
        ...
        coinControlDialog->addPayAmount(entry->getAmountValue());
    }
}
...
void SendWidget::onMuBdISelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto MuBdIAdd = Standard::DecodeDestination(address.toStdString(), isStakingAddr);

        if (!Standard::IsValidDestination(MuBdIAdd) || isStakingAddr) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(MuBdIAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
...
void SendWidget::setCustomFeeSelected(bool isSelected, const CAmount& customFee)
{
    ...
    ui->pushButtonFee->setText(isCustomFeeSelected ?
                    tr("Custom Fee %1").arg(MuBdIUnits::formatWithUnit(nDisplayUnit, customFee) + "/kB") :
                    tr("Customize Fee"));
    if (walletModel)
