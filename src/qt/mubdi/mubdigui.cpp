...
void MuBdIGUI::createActions(const NetworkStyle* networkStyle)
{
    ...
    connect(toggleHideAction, &QAction::triggered, this, &MuBdIGUI::toggleHidden);
...
void MuBdIGUI::connectActions()
{
    ...
    connect(governancewidget, &GovernanceWidget::showHide, this, &MuBdIGUI::showHide);
    connect(governancewidget, &GovernanceWidget::execDialog, this, &MuBdIGUI::execDialog);
...
void MuBdIGUI::setClientModel(ClientModel* _clientModel)
{
    ...
    if (this->clientModel) {
        ...
        connect(clientModel, &ClientModel::message, this, &MuBdIGUI::message);
...
void MuBdIGUI::createTrayIconMenu()
{
    ...
    connect(trayIcon, &QSystemTrayIcon::activated, this, &MuBdIGUI::trayIconActivated);
#else
    // Note: On macOS, the Dock icon is used to provide the tray's functionality.
    MacDockIconHandler* dockIconHandler = MacDockIconHandler::instance();
    connect(dockIconHandler, &MacDockIconHandler::dockIconClicked, this, &MuBdIGUI::macosDockIconActivated);
...
#else
void MuBdIGUI::macosDockIconActivated()
 {
     show();
     activateWindow();
 }
    ...
    if (e->type() == QEvent::WindowStateChange) {
        if (clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray()) {
            ...
            if (!(wsevt->oldState() & Qt::WindowMinimized) && isMinimized()) {
                QTimer::singleShot(0, this, &MuBdIGUI::hide);
                e->ignore();
            } else if ((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized()) {
                QTimer::singleShot(0, this, &MuBdIGUI::show);
                e->ignore();
            }
        }
    }
...
void MuBdIGUI::goToSettingsInfo()
{
    navMenu->selectSettings();
...
void MuBdIGUI::openNetworkMonitor()
{
...
void MuBdIGUI::openFAQ(SettingsFaqWidget::Section section)
{
    showHide(true);
...
#ifdef ENABLE_WALLET
void MuBdIGUI::setGovModel(GovernanceModel* govModel)
{
    if (!stackedContainer || !clientModel) return;
    governancewidget->setGovModel(govModel);
}

void MuBdIGUI::setMNModel(MNModel* mnModel)
{
    if (!stackedContainer || !clientModel) return;
...
bool MuBdIGUI::addWallet(const QString& name, WalletModel* walletModel)
{
    ...
    // Connect actions..
    connect(walletModel, &WalletModel::message, this, &MuBdIGUI::message);
    connect(masterNodesWidget, &MasterNodesWidget::message, this, &MuBdIGUI::message);
    connect(coldStakingWidget, &ColdStakingWidget::message, this, &MuBdIGUI::message);
    ...
    connect(governancewidget, &GovernanceWidget::message,this, &MuBdIGUI::message);
    connect(settingsWidget, &SettingsWidget::message, this, &MuBdIGUI::message);

    // Pass through transaction notifications
    connect(dashboard, &DashboardWidget::incomingTransaction, this, &MuBdIGUI::incomingTransaction);
