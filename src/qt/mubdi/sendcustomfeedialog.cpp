...
SendCustomFeeDialog::SendCustomFeeDialog(MuBdIGUI* parent, WalletModel* model) :
    ...
    walletModel(model)
{
    ...
    if (parent)
        connect(parent, &MuBdIGUI::themeChanged, this, &SendCustomFeeDialog::onChangeTheme);
...
void SendCustomFeeDialog::showEvent(QShowEvent* event)
{
    ...
    ui->labelCustomFee->setText(MuBdIUnits::name(walletModel->getOptionsModel()->getDisplayUnit()) + "/kB");
...
void SendCustomFeeDialog::onCustomChecked()
{
    ...
    if (isChecked) {
        ...
        ui->lineEditCustomFee->setText(MuBdIUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), nFee));
    } else {
...
void SendCustomFeeDialog::accept()
{
    ...
    // Check insane fee
    const CAmount insaneFee = ::minRelayTxFee.GetFeePerK() * 5000;
    if (customFee >= insaneFee) {
        ui->lineEditCustomFee->setText(MuBdIUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), insaneFee - walletModel->getNetMinFee()));
        inform(tr("Fee too high. Must be below: %1").arg(
                MuBdIUnits::formatWithUnit(walletModel->getOptionsModel()->getDisplayUnit(), insaneFee)));
        ...
        inform(tr("Fee too low. Must be at least: %1").arg(
                MuBdIUnits::formatWithUnit(walletModel->getOptionsModel()->getDisplayUnit(), walletModel->getNetMinFee())));
    } else {
