    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopMuBdI, ui->labelAmountTopShieldedMuBdI}, "amount-small-topbar");
    setCssProperty({ui->labelAmountMuBdI}, "amount-topbar");
    setCssProperty({ui->labelPendingMuBdI, ui->labelImmatureMuBdI}, "amount-small-topbar");
...
void TopBar::onBtnBalanceInfoClicked()
{
    ...
    balanceBubble->updateValues(balances.balance, nDisplayUnit);
    QPoint pos = this->pos();
...
void TopBar::updateBalances(const interfaces::WalletBalances& newBalance)
{
    ...
    // MuBdI Total
    QString totalMuBdI = GUIUtil::formatBalance(newBalance.balance, nDisplayUnit);
    QString totalTransparent = GUIUtil::formatBalance(newBalance.balance);

    // MuBdI
    // Top
    ui->labelAmountTopMuBdI->setText(totalTransparent);
    ui->labelAmountTopShieldedMuBdI->setText(GUIUtil::formatBalance(newBalance.balance * 2000));
    // Expanded
    ui->labelAmountMuBdI->setText(totalMuBdI);
    ui->labelPendingMuBdI->setText(GUIUtil::formatBalance(newBalance.unconfirmed_balance, nDisplayUnit));
    ui->labelImmatureMuBdI->setText(GUIUtil::formatBalance(newBalance.immature_balance, nDisplayUnit));
}
