...
{
    try {
        const QString& latestAddress = (refreshAddress.isEmpty()) ? addressTableModel->getAddressToShow() : refreshAddress;
...
void ReceiveWidget::onNewAddressClicked()
{
    try {
        ...
        CallResult<Destination> r = walletModel->getNewAddress("");
