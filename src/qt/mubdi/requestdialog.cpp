...
void RequestDialog::setWalletModel(WalletModel *model)
{
    ...
    ui->comboBoxCoin->setText(MuBdIUnits::name(this->walletModel->getOptionsModel()->getDisplayUnit()));
}
...
void RequestDialog::accept()
{
    if (walletModel) {
        ...
        if (this->isPaymentRequest) {
            ...
            title = tr("Request for ") + MuBdIUnits::format(displayUnit, info->amount, false, MuBdIUnits::separatorAlways) + " " + MuBdIUnits::name(displayUnit);
        } else {
