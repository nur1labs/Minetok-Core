...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &MuBdIGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isMuBdI = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::MNReward;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair MuBdI, MN Reward
...
void DashboardWidget::onChartRefreshed()
{
    ...
    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalMuBdI > 0 || chartData->totalMN > 0) {
    ...
    forceUpdateStyle({ui->labelAmountMuBdI, ui->labelAmountMN});
