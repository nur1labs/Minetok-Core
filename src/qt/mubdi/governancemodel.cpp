...
OperationResult GovernanceModel::validatePropAmount(CAmount amount) const
{
    if (amount < PROPOSAL_MIN_AMOUNT) { // Future: move constant to a budget interface.
        return {false, strprintf("Amount below the minimum of %s RCI", FormatMoney(PROPOSAL_MIN_AMOUNT))};
    }

    if (amount > getMaxAvailableBudgetAmount()) {
        return {false, strprintf("Amount exceeding the maximum available of %s RCI", FormatMoney(getMaxAvailableBudgetAmount()))};
    }
    return {true};
}
