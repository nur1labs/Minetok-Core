...
bool CMasternodeDB::Write(const CMasternodeMan& mnodemanToSave)
{
    ...
    LogPrint(AILog::MASTERNODE,"Written info to mncache.dat  %dms\n", GetTimeMillis() - nStart);
...
CMasternodeDB::ReadResult CMasternodeDB::Read(CMasternodeMan& mnodemanToLoad)
{
    ...
    LogPrint(AILog::MASTERNODE,"Loaded info from mncache.dat (dbversion=%d) %dms\n", version, GetTimeMillis() - nStart);
...
void DumpMasternodes()
{
    ...
    CMasternodeDB mndb;
    LogPrint(AILog::MASTERNODE,"Writing info to mncache.dat...\n");
    mndb.Write(mnodeman);

    LogPrint(AILog::MASTERNODE,"Masternode dump finished  %dms\n", GetTimeMillis() - nStart);
}
...
bool CMasternodeMan::Add(CMasternode& mn)
{
    ...
    if (it == mapMasternodes.end()) {
        LogPrint(AILog::MASTERNODE, "Adding new Masternode %s\n", mn.vin.prevout.ToString());
        mapMasternodes.emplace(mn.vin.prevout, std::make_shared<CMasternode>(mn));
        LogPrint(AILog::MASTERNODE, "Masternode added. New total count: %d\n", mapMasternodes.size());
        return true;
    }
...
void CMasternodeMan::AskForMN(CNode* pnode, const CTxIn& vin)
{
    ...
    LogPrint(AILog::MASTERNODE, "CMasternodeMan::AskForMN - Asking node for missing entry, vin: %s\n", vin.prevout.hash.ToString());
...
int CMasternodeMan::CheckAndRemove(bool forceExpiredRemoval)
{
    ...
    while (it != mapMasternodes.end()) {
        ...
        if (activeState == CMasternode::MASTERNODE_REMOVE ||
            ...
            LogPrint(AILog::MASTERNODE, "Removing inactive (legacy) Masternode %s\n", it->first.ToString());
            ...
            LogPrint(AILog::MASTERNODE, "Masternode removed.\n");
        } else {
            ++it;
        }
    }
    LogPrint(AILog::MASTERNODE, "New total masternode count: %d\n", mapMasternodes.size());
...
CMasternodeMan::MNsInfo CMasternodeMan::getMNsInfo() const
{
    ...
    bool spork_3_active = sporkManager.IsSporkActive(SPORK_3_MASTERNODE_PAYMENT_ENFORCEMENT);
    ...
    // legacy masternodes
    {
        ...
        for (const auto& it : mapMasternodes) {
            ...
            if (spork_3_active && (GetAdjustedTime() - mn->sigTime < MN_WINNER_MINIMUM_AGE)) {
        ...
        if (!(pnode->addr.IsRFC1918() || pnode->addr.IsLocal())) {
            ...
            if (it != mWeAskedForMasternodeList.end()) {
                if (GetTime() < (*it).second) {
                    LogPrint(AILog::MASTERNODE, "dseg - we already asked peer %i for the list; skipping...\n", pnode->GetId());
                    return false;
                }
            }
        }
    ...
    std::vector<std::pair<int64_t, CTxIn> > vecMasternodeScores;
    {
        ...
        for (const auto& it : mapMasternodes) {
            ...
            if (mn->protocolVersion < minProtocol) {
                LogPrint(AILog::MASTERNODE,"Skipping Masternode with obsolete version %d\n", mn->protocolVersion);
...
bool CMasternodeMan::CheckInputs(CMasternodeBroadcast& mnb, int nChainHeight, int& nDoS)
{
    ...
    if (collateralUtxo.IsSpent()) {
        LogPrint(AILog::MASTERNODE,"mnb - vin %s spent\n", mnb.vin.prevout.ToString());
        return false;
    }

    // Check collateral value
    if (collateralUtxo.out.nValue != Params().GetConsensus().nMNCollateralAmt) {
        LogPrint(AILog::MASTERNODE,"mnb - invalid amount for mnb collateral %s\n", mnb.vin.prevout.ToString());
        nDoS = 33;
        return false;
    }
    ...
    if (collateralUtxo.out.scriptPubKey != payee) {
        LogPrint(AILog::MASTERNODE,"mnb - collateral %s not associated with mnb pubkey\n", mnb.vin.prevout.ToString());
        nDoS = 33;
        return false;
    }

    LogPrint(AILog::MASTERNODE, "mnb - Accepted Masternode entry\n");
    ...
    if (collateralUtxoDepth < MasternodeCollateralMinConf()) {
        LogPrint(AILog::MASTERNODE,"mnb - Input must have at least %d confirmations\n", MasternodeCollateralMinConf());
    ...
    // should be at least not earlier than block when 1000 MUB tx got MASTERNODE_MIN_CONFIRMATIONS
    ...
    if (pConfIndex->GetBlockTime() > mnb.sigTime) {
        LogPrint(AILog::MASTERNODE,"mnb - Bad sigTime %d for Masternode %s (%i conf block is at %d)\n",
...
int CMasternodeMan::ProcessMNBroadcast(CNode* pfrom, CMasternodeBroadcast& mnb)
{
    ...
    LogPrint(AILog::MASTERNODE,"%s - Got NEW Masternode entry - %s - %lli \n", __func__,
             mnb.vin.prevout.hash.ToString(), mnb.sigTime);
    CMasternode mn(mnb);
    if (!Add(mn)) {
        LogPrint(AILog::MASTERNODE, "%s - Rejected Masternode entry %s\n", __func__,
...
int CMasternodeMan::ProcessGetMNList(CNode* pfrom, CTxIn& vin)
{
    ...
    if (!vin.IsNull()) {
        ...
        LogPrint(AILog::MASTERNODE, "dseg - Sent 1 Masternode entry to peer %i\n", pfrom->GetId());
        return 0;
    }
    ...
    int nInvCount = 0;
    {
        ...
        for (auto& it : mapMasternodes) {
            ...
            if (mn->IsEnabled()) {
                LogPrint(AILog::MASTERNODE, "dseg - Sending Masternode entry - %s \n", mn->vin.prevout.hash.ToString());
    ...
    LogPrint(AILog::MASTERNODE, "dseg - Sent %d Masternode entries to peer %i\n", nInvCount, pfrom->GetId());
...
int CMasternodeMan::ProcessMessageInner(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == NetMsgType::MNBROADCAST) {
        ...
        if (mnb.addr.IsAddrV1Compatible()) {
            LogPrint(AILog::MASTERNODE, "%s: mnb2 with pre-BIP155 node addr format rejected\n", __func__);
            return 30;
        }
        ...
        CMasternodePing mnp;
        vRecv >> mnp;
        LogPrint(AILog::MNPING, "mnp - Masternode ping, vin: %s\n", mnp.vin.prevout.hash.ToString());
        {
            // Clear inv request
            LOCK(cs_main);
...
void CMasternodeMan::UpdateMasternodeList(CMasternodeBroadcast& mnb)
{
    ...
    LogPrint(AILog::MASTERNODE,"%s -- masternode=%s\n", __func__, mnb.vin.prevout.ToString());
...
uint256 CMasternodeMan::GetHashAtHeight(int nHeight) const
{
    ...
    if (nHeight < 0) {
        LogPrint(AILog::MASTERNODE, "%s: Negative height. Returning 0\n",  __func__);
        return UINT256_ZERO;
    }
    int nCurrentHeight = GetBestHeight();
    if (nHeight > nCurrentHeight) {
        LogPrint(AILog::MASTERNODE, "%s: height %d over current height %d. Returning 0\n",
...
void ThreadCheckMasternodes()
{
    ...
    util::ThreadRename("mubdi-masternodeman");
