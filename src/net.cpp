...
static std::vector<CAddress> ConvertSeeds(const std::vector<uint8_t> &vSeedsIn)
{
    ...
    while (!s.eof()) {
        ...
        LogPrint(AILog::NET, "Added hardcoded seed: %s\n", addr.ToString());
...
CNode* CConnman::ConnectNode(CAddress addrConnect, const char* pszDest, bool fCountFailure)
{
    ...
    /// debug print
    LogPrint(AILog::NET, "trying connection %s lastseen=%.1fhrs\n",
    ...
    if (pszDest) {
        ...
        if (Lookup(pszDest, resolved,  default_port, fNameLookup && !HaveNameProxy(), 256) && !resolved.empty()) {
            ...
            if (!addrConnect.IsValid()) {
                LogPrint(AILog::NET, "Resolver returned invalid address %s for %s", addrConnect.ToString(), pszDest);
                return nullptr;
            }
        }
    }
...
void CConnman::DumpBanlist()
{
    ...
    LogPrint(AILog::NET, "Flushed %d banned node ips/subnets to banlist.dat  %dms\n",
...
void CNode::CloseSocketDisconnect()
{
    ...
    LOCK(cs_hSocket);
    if (hSocket != INVALID_SOCKET) {
        LogPrint(AILog::NET, "disconnecting peer=%d\n", id);
        CloseSocket(hSocket);
    }
}
    ...
    bool notifyUI = false;
    {
        ...
        while(it != setBanned.end())
        {
            ...
            if(now > banEntry.nBanUntil)
            {
                ...
                setBannedIsDirty = true;
                notifyUI = true;
                LogPrint(AILog::NET, "%s: Removed banned node ip/subnet from banlist.dat: %s\n", __func__, subNet.ToString());
            }
            else
                ++it;
        }
...
bool CNode::ReceiveMsgBytes(const char* pch, unsigned int nBytes, bool& complete)
{
    ...
    while (nBytes > 0) {
        ...
        if (msg.in_data && msg.hdr.nMessageSize > MAX_PROTOCOL_MESSAGE_LENGTH) {
            LogPrint(AILog::NET, "Oversized message from peer=%i, disconnecting\n", GetId());
            return false;
        }
...
void CConnman::AcceptConnection(const ListenSocket& hListenSocket) {
    ...
    if (IsBanned(addr) && !whitelisted) {
        LogPrint(AILog::NET, "connection from %s dropped (banned)\n", addr.ToString());
        CloseSocket(hSocket);
        return;
    }
    ...
    if (nInbound >= nMaxConnections - MAX_OUTBOUND_CONNECTIONS) {
        if (!AttemptToEvictConnection(whitelisted)) {
            ...
            LogPrint(AILog::NET, "failed to find an eviction candidate - connection dropped (full)\n");
            CloseSocket(hSocket);
            return;
        }
    }
    ...
    LogPrint(AILog::NET, "connection from %s accepted\n", addr.ToString());
        ...
        for (CNode* pnode : vNodesCopy) {
                ...
                {
                    {
                        ...
                        if (nBytes > 0) {
                            ...
                            if (!pnode->fDisconnect)
                                LogPrint(AILog::NET, "socket closed\n");
                            pnode->CloseSocketDisconnect();
                        } else if (nBytes < 0) {
                            // error
                            int nErr = WSAGetLastError();
            ...
            if (nTime - pnode->nTimeConnected > 60) {
                if (pnode->nLastRecv == 0 || pnode->nLastSend == 0) {
                    LogPrint(AILog::NET, "socket no message in first 60 seconds, %d %d from %d\n", pnode->nLastRecv != 0, pnode->nLastSend != 0, pnode->GetId());
                    pnode->fDisconnect = true;
...
void CConnman::DumpAddresses()
{
    ...
    LogPrint(AILog::NET, "Flushed %d addresses to peers.dat  %dms\n",
        ...
        if (addrConnect.IsValid()) {
            if (fFeeler) {
                ...
                if (!interruptNet.sleep_for(std::chrono::milliseconds(randsleep)))
                    return;
                LogPrint(AILog::NET, "Making feeler connection to %s\n", addrConnect.ToString());
            }
...
bool CConnman::Start(CScheduler& scheduler, std::string& strNodeError, Options connOptions)
{
    ...
    if (bandb.Read(banmap)) {
        ...
        LogPrint(AILog::NET, "Loaded %d banned node ips/subnets from banlist.dat  %dms\n",
            banmap.size(), GetTimeMillis() - nStart);
    } else {
    ...
    nSendVersion(0)
{
    ...
    if (fLogIPs)
        LogPrint(AILog::NET, "Added connection to %s peer=%d\n", addrName, id);
    else
        LogPrint(AILog::NET, "Added connection peer=%d\n", id);
}
...
void CConnman::PushMessage(CNode* pnode, CSerializedNetMsg&& msg)
{
    ...
    LogPrint(AILog::NET, "sending %s (%d bytes) peer=%d\n",  SanitizeString(msg.command.c_str()), nMessageSize, pnode->GetId());
