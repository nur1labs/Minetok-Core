...
std::string HelpMessageCli()
{
    ...
    strUsage += HelpMessageOpt("-rpcwallet=<walletname>", "Send RPC for non-default wallet on RPC server (needs to exactly match corresponding -wallet option passed to mubdid)");
...
static bool AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc < 2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") || gArgs.IsArgSet("-help") || gArgs.IsArgSet("-version")) {
        ...
        if (!gArgs.IsArgSet("-version")) {
            strUsage += "\n"
                        "Usage:  mubdi-cli [options] <command> [params]  Send command to " PACKAGE_NAME "\n"
                        "or:     mubdi-cli [options] -named <command> [name=value]... Send command to " PACKAGE_NAME " (with named arguments)\n"
                        "or:     mubdi-cli [options] help                List commands\n"
                        "or:     mubdi-cli [options] help <command>      Get help for a command\n";
            strUsage += "\n" + HelpMessageCli();
        }
