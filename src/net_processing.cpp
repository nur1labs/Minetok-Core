...
void PushNodeVersion(CNode* pnode, CConnman* connman, int64_t nTime)
{
    ...
    if (fLogIPs)
        LogPrint(AILog::NET, "send version message: version %d, blocks=%d, us=%s, them=%s, peer=%d\n", PROTOCOL_VERSION, nNodeStartingHeight, addrMe.ToString(), addrYou.ToString(), nodeid);
    else
        LogPrint(AILog::NET, "send version message: version %d, blocks=%d, us=%s, peer=%d\n", PROTOCOL_VERSION, nNodeStartingHeight, addrMe.ToString(), nodeid);
}
...
bool AddOrphanTx(const CTransactionRef& tx, NodeId peer) EXCLUSIVE_LOCKS_REQUIRED(g_cs_orphans)
{
    ...
    if (sz >= nMaxSize) {
        LogPrint(AILog::MEMPOOL, "ignoring large orphan tx (size: %u, hash: %s)\n", sz, hash.ToString());
        return false;
    }
    ...
    LogPrint(AILog::MEMPOOL, "stored orphan tx %s (mapsz %u outsz %u)\n", hash.ToString(),
...
void EraseOrphansFor(NodeId peer)
{
    ...
    if (nErased > 0) LogPrint(AILog::MEMPOOL, "Erased %d orphan tx from peer %d\n", nErased, peer);
}
...
unsigned int LimitOrphanTxSize(unsigned int nMaxOrphans)
{
    ...
    if (nNextSweep <= nNow) {
        ...
        if (nErased > 0) LogPrint(AILog::MEMPOOL, "Erased %d orphan tx due to expiration\n", nErased);
    }
...
void Misbehaving(NodeId pnode, int howmuch, const std::string& message) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if (state->nMisbehavior >= banscore && state->nMisbehavior - howmuch < banscore) {
        LogPrint(AILog::NET, "%s: %s peer=%d (%d -> %d) BAN THRESHOLD EXCEEDED%s\n", __func__, state->name, pnode, state->nMisbehavior-howmuch, state->nMisbehavior, message_prefixed);
        state->fShouldBan = true;
    } else {
        LogPrint(AILog::NET, "%s: %s peer=%d (%d -> %d)%s\n", __func__, state->name, pnode, state->nMisbehavior-howmuch, state->nMisbehavior, message_prefixed);
    }
}
...
void PeerLogicValidation::BlockConnected(const std::shared_ptr<const CBlock>& pblock, const CBlockIndex* pindex)
{
    ...
    if (!vOrphanErase.empty()) {
        ...
        LogPrint(AILog::MEMPOOL, "Erased %d orphan tx included or conflicted by block\n", nErased);
    }
}
...
void static ProcessGetBlockData(CNode* pfrom, const CInv& inv, CConnman* connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (mi != mapBlockIndex.end()) {
        if (chainActive.Contains(mi->second)) {
            ...
            if (!send) {
                LogPrint(AILog::NET, "ProcessGetData(): ignoring request from peer=%i for old block that isn't in the main chain\n", pfrom->GetId());
            }
        }
    }
...
{
    LogPrint(AILog::NET, "received: %s (%u bytes) peer=%d\n", SanitizeString(strCommand), vRecv.size(), pfrom->GetId());
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        if (pfrom->nServicesExpected & ~nServices) {
            LogPrint(AILog::NET, "peer=%d does not offer the expected services (%08x offered, %08x expected); disconnecting\n", pfrom->GetId(), nServices, pfrom->nServicesExpected);
        ...
        LogPrint(AILog::NET, "receive version message: %s: version %d, blocks=%d, us=%s, peer=%d%s\n",
        ...
        bool fMissingSporks = !pSporkDB->SporkExists(SPORK_6_NEW_PROTOCOL_ENFORCEMENT) ||
                              !pSporkDB->SporkExists(SPORK_7_NEW_PROTOCOL_ENFORCEMENT_2) ||
                              !pSporkDB->SporkExists(SPORK_8_COLDSTAKING_ENFORCEMENT) ||
                              !pSporkDB->SporkExists(SPORK_10_BLOCKSTREAM_ENCHANCEMENT) ||
                              !pSporkDB->SporkExists(SPORK_11_GENESIS_ENCHANCEMENT);
    ...
    if (strCommand == NetMsgType::VERACK) {
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++) {
            ...
            LogPrint(AILog::NET, "got inv: %s  %s peer=%d\n", inv.ToString(), fAlreadyHave ? "have" : "new", pfrom->GetId());
            ...
            if (inv.type == MSG_BLOCK) {
                ...
                if (!fAlreadyHave && !fImporting && !fReindex && !mapBlocksInFlight.count(inv.hash)) {
                    ...
                    LogPrint(AILog::NET, "getblocks (%d) %s to peer=%d\n", pindexBestHeader->nHeight, inv.hash.ToString(), pfrom->GetId());
                }
            } else {
        ...
        if (vInv.size() != 1)
            LogPrint(AILog::NET, "received getdata (%u invsz) peer=%d\n", vInv.size(), pfrom->GetId());

        if (vInv.size() > 0)
            LogPrint(AILog::NET, "received getdata for: %s peer=%d\n", vInv[0].ToString(), pfrom->GetId());
        ...
        if (locator.vHave.size() > MAX_LOCATOR_SZ) {
            LogPrint(AILog::NET, "getblocks locator size %lld > %d, disconnect peer=%d\n", locator.vHave.size(), MAX_LOCATOR_SZ, pfrom->GetId());
            pfrom->fDisconnect = true;
            return true;
        }
        ...
        int nLimit = 500;
        LogPrint(AILog::NET, "getblocks %d to %s limit %d from peer=%d\n", (pindex ? pindex->nHeight : -1), hashStop.IsNull() ? "end" : hashStop.ToString(), nLimit, pfrom->GetId());
        for (; pindex; pindex = chainActive.Next(pindex)) {
            if (pindex->GetBlockHash() == hashStop) {
                LogPrint(AILog::NET, "  getblocks stopping at %d %s\n", pindex->nHeight, pindex->GetBlockHash().ToString());
                break;
            }
            ...
            if (--nLimit <= 0) {
                ...
                LogPrint(AILog::NET, "  getblocks stopping at limit %d %s\n", pindex->nHeight, pindex->GetBlockHash().ToString());
        ...
        if (AcceptToMemoryPool(mempool, state, ptx, true, &fMissingInputs, false, ignoreFees)) {
            ...
            LogPrint(AILog::MEMPOOL, "%s : peer=%d %s : accepted %s (poolsz %u txn, %u kB)\n",
            ...
            while (!vWorkQueue.empty()) {
                ...
                for (auto mi = itByPrev->second.begin();
                    ...
                    if (AcceptToMemoryPool(mempool, stateDummy, orphanTx, true, &fMissingInputs2)) {
                        LogPrint(AILog::MEMPOOL, "   accepted orphan tx %s\n", orphanHash.ToString());
                        ...
                        if(stateDummy.IsInvalid(nDos) && nDos > 0) {
                            ...
                            LogPrint(AILog::MEMPOOL, "   invalid orphan tx %s\n", orphanHash.ToString());
                        }
                        // Has inputs but not accepted to mempool
                        // Probably non-standard or insufficient fee
                        LogPrint(AILog::MEMPOOL, "   removed orphan tx %s\n", orphanHash.ToString());
                        vEraseQueue.push_back(orphanHash);
                        assert(recentRejects);
            ...
            if (!fRejectedParents) {
                ...
                if (nEvicted > 0)
                    LogPrint(AILog::MEMPOOL, "mapOrphan overflow, removed %u tx\n", nEvicted);
            } else {
                LogPrint(AILog::MEMPOOL, "not keeping orphan with rejected parents %s\n",tx.GetHash().ToString());
            }
        } else {
        ...
        int nDoS = 0;
        if (state.IsInvalid(nDoS)) {
            LogPrint(AILog::MEMPOOLREJ, "%s from peer=%d %s was not accepted into the memory pool: %s\n", tx.GetHash().ToString(),
            ...
            if (!AcceptBlockHeader((CBlock)header, state, &pindexLast)) {
                ...
                if (state.IsInvalid(nDoS)) {
                    if (nDoS > 0) {
                        ...
                        LogPrint(AILog::NET, "peer=%d: invalid header received\n", pfrom->GetId());
                    }
                    return false;
                }
            }
    ...
    else if (strCommand == NetMsgType::BLOCK && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        LogPrint(AILog::NET, "received block %s peer=%d\n", inv.hash.ToString(), pfrom->GetId());
        ...
        if (!mapBlockIndex.count(pblock->hashPrevBlock)) {
            ...
            if (!mapBlockIndex.count(hashBlock)) {
                ...
                LogPrint(AILog::NET, "%s : Already processed block %s, skipping ProcessNewBlock()\n", __func__, pblock->GetHash().GetHex());
            }
        }
    }
    ...
    // getaddr message mitigates the attack.
        ...
        if (!(pfrom->GetLocalServices() & NODE_BLOOM) && !pfrom->fWhitelisted) {
            LogPrint(AILog::NET, "mempool request with bloom filters disabled, disconnect peer=%d\n", pfrom->GetId());
            pfrom->fDisconnect = true;
            return true;
        }
        ...
        if (!(sProblem.empty())) {
            LogPrint(AILog::NET, "pong peer=%d %s: %s, %x expected, %x received, %u bytes\n",
                pfrom->GetId(),
                pfrom->cleanSubVer,
                sProblem,
        ...
        if (std::find(allMessages.begin(), allMessages.end(), strCommand) != allMessages.end()) {
            ...
            LogPrint(AILog::NET, "Unknown command \"%s\" from peer=%d\n", SanitizeString(strCommand), pfrom->GetId());
        }
    }
...
bool PeerLogicValidation::ProcessMessages(CNode* pfrom, std::atomic<bool>& interruptMsgProc)
{
    ...
    if (memcmp(msg.hdr.pchMessageStart, Params().MessageStart(), MESSAGE_START_SIZE) != 0) {
        LogPrint(AILog::NET, "PROCESSMESSAGE: INVALID MESSAGESTART %s peer=%d\n", SanitizeString(msg.hdr.GetCommand()), pfrom->GetId());
        pfrom->fDisconnect = true;
        return false;
    }
    ...
    if (!hdr.IsValid(Params().MessageStart())) {
        LogPrint(AILog::NET, "PROCESSMESSAGE: ERRORS IN HEADER '%s' peer=%d\n", SanitizeString(hdr.GetCommand()), pfrom->GetId());
        return fMoreWork;
    }
    ...
    {
        LogPrint(AILog::NET, "%s(%s, %u bytes): CHECKSUM ERROR expected %s was %s\n", __func__,
    ...
    try {
        ...
        if (strstr(e.what(), "end of data")) {
            ...
            LogPrint(AILog::NET, "ProcessMessages(%s, %u bytes): Exception '%s' caught, normally caused by a message being shorter than its stated length\n", SanitizeString(strCommand), nMessageSize, e.what());
        } else if (strstr(e.what(), "size too large")) {
            // Allow exceptions from over-long size
            LogPrint(AILog::NET, "ProcessMessages(%s, %u bytes): Exception '%s' caught\n", SanitizeString(strCommand), nMessageSize, e.what());
        } else {
    ...
    if (!fRet)
        LogPrint(AILog::NET, "ProcessMessage(%s, %u bytes) FAILED peer=%d\n", SanitizeString(strCommand), nMessageSize, pfrom->GetId());
...
{
    {
        ...
        if (!state.fSyncStarted && !pto->fClient && !fImporting && !fReindex) {
            ...
            if ((nSyncStarted == 0 && fFetch) || pindexBestHeader->GetBlockTime() > GetAdjustedTime() - 6 * 60 * 60) { // NOTE: was "close to today" and 24h in Bitcoin
                ...
                //LogPrint(AILog::NET, "initial getheaders (%d) to peer=%d (startheight:%d)\n", pindexStart->nHeight, pto->GetId(), pto->nStartingHeight);
        ...
        if (!pto->fClient && fFetch && state.nBlocksInFlight < MAX_BLOCKS_IN_TRANSIT_PER_PEER) {
            ...
            if (state.nBlocksInFlight == 0 && staller != -1) {
                if (State(staller)->nStallingSince == 0) {
                    ...
                    LogPrint(AILog::NET, "Stall started peer=%d\n", staller);
                }
            }
        }
        ...
        while (!pto->mapAskFor.empty() && (*pto->mapAskFor.begin()).first <= nNow) {
            ...
            if (!AlreadyHave(inv)) {
                LogPrint(AILog::NET, "Requesting %s peer=%d\n", inv.ToString(), pto->GetId());
