...
bool CreateCoinbaseTx(CBlock* pblock, const CScript& scriptPubKeyIn, CBlockIndex* pindexPrev)
{
    ...
    FillBlockPayee(txNew, txDummy, pindexPrev, false);
...
bool SolveProofOfStake(CBlock* pblock, CBlockIndex* pindexPrev, CWallet* pwallet, std::vector<CStakeableOutput>* availableCoins)
{
    ...
    if (!pwallet->CreateCoinStake(pindexPrev, pblock->nBits, txCoinStake, nTxNewTime, availableCoins)) {
        LogPrint(AILog::STAKING, "%s : stake not found\n", __func__);
        return false;
    }
    // Stake found
...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake, std::vector<CStakeableOutput>* availableCoins)
{
...

    {
        ...
        for (CTxMemPool::indexed_transaction_set::iterator mi = mempool.mapTx.begin();
            ...
            if (tx.IsCoinBase() || tx.IsCoinStake() || !IsFinalTx(mi->GetSharedTx(), nHeight)){
                continue;
            }
            ...
            mempool.ApplyDelta(hash, dPriority, nTotalIn);
        ...
        while (!vecPriority.empty()) {
            ...
            CAmount nFeeDelta = 0;
            mempool.ApplyDelta(hash, dPriorityDelta, nFeeDelta);
        ...
        /* Test POW for now disable cause POS merge
        CValidationState state;
...
CBlockTemplate* CreateNewBlockWithScript(const CScript& coinbaseScript, CWallet* pwallet)
{
    ...
    return CreateNewBlock(coinbaseScript, pwallet, false);
}
...
void GenerateMuBdIs(bool fGenerate, CWallet* pwallet, int nThreads)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(std::bind(&ThreadMuBdIMiner, pwallet));
}
