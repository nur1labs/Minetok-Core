...
public:
     ...
     *   * MuBdI Core version N knows how to parse up to format=3. If a new format=4 is
     *     introduced in version N+1 that is compatible with format=3 and it is known that
     *     version N will be able to parse it, then version N+1 will write
     *     (format=4, lowest_compatible=3) in the first two bytes of the file, and so
     *     version N will still try to parse it.
     *   * MuBdI Core version N+2 introduces a new incompatible format=5. It will write
    ...
    void Unserialize(Stream& s_)
    {
        ...
        for (int n = 0; n < nNew; n++) {
            ...
            if (format >= Format::V2_ASMAP && nUBuckets == ADDRMAN_NEW_BUCKET_COUNT && vvNew[bucket][nUBucketPos] == -1 &&
                ...
                LogPrint(AILog::ADDRMAN, "Bucketing method was updated, re-bucketing addrman entries from disk\n");
        ...
        if (nLost + nLostUnk > 0) {
            LogPrint(AILog::ADDRMAN, "addrman lost %i new and %i tried addresses due to collisions\n", nLostUnk, nLost);
        }
    ...
    bool Add(const CAddress& addr, const CNetAddr& source, int64_t nTimePenalty = 0)
    {
        ...
        Check();
        if (fRet) {
            LogPrint(AILog::ADDRMAN, "Added %s from %s: %i tried, %i new\n", addr.ToStringIPPort(), source.ToString(), nTried, nNew);
        }
        return fRet;
    }
    ...
    bool Add(const std::vector<CAddress>& vAddr, const CNetAddr& source, int64_t nTimePenalty = 0)
    {
        ...
        Check();
        if (nAdd) {
            LogPrint(AILog::ADDRMAN, "Added %i addresses from %s: %i tried, %i new\n", nAdd, source.ToString(), nTried, nNew);
        }
        return nAdd > 0;
    }
