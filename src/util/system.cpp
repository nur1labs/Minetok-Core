...
const char * const MUBDI_CONF_FILENAME = "minetok.conf";
const char * const MUBDI_PID_FILENAME = "minetok.pid";
const char * const MUBDI_MASTERNODE_CONF_FILENAME = "minetok-mn.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager& am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "mubdid -foo=bar
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MineTok";
#else
    fs::path pathRet;
